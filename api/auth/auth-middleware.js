  ...
  */
    try {
      ...
      if(!authHeader){
        ...
        jwt.verify(authHeader,JWT_SECRET,(err,decodedToken)=>{// verify etmek, secreta göre tokenı aç ve tokenın decode edilmiş halini bana getir demek. decode edilemiyorsa err getirir (jwt secretı yanlış girdiğin örneğin) (sign loginde kullanılıyor)
          if(err){
            res.status(401).json({message:"token gecersizdir"});
          }else{
            req.decodedToken = decodedToken;// Alt akıştaki middlewarelar için hayatı kolaylaştırmak için kodu çözülmüş tokeni req nesnesine koyun!
            next();
          }
        })
      }
...
const sadece = (role_name) => (req, res, next) => {
  ...
  */
    try {
      if(req.decodedToken.role_name === role_name){//req.decodedToken bunu zaten sinirli middleware içinde yarattık, tokenın içindeki payload, options ve secretı açtık
        next();
      }else{
